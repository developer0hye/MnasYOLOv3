...
class MnasYOLOv3(nn.Module):
    ...
    def yololoss(self,
        ...
        n_pos = len(torch.nonzero(t_obj == 1.0))
        n_neg = len(torch.nonzero(t_obj == 0.0))

        # print(n_pos)

        loss_xy = bbox_w * nn.BCEWithLogitsLoss(reduction='sum')(p_bbox_xy[t_obj == 1.0], t_bbox_xy[t_obj == 1.0])/n_pos
        #loss_xy_new =  bbox_w * torch.sum(loss_xy_func(p_bbox_xy[t_obj == 1.0], t_bbox_xy[t_obj == 1.0]), 1).sum() / n_pos

        #print(torch.mean(torch.sum(torch.sum(loss_xy_func(p_bbox_xy, t_bbox_xy), 2) * t_obj, 1)))
        #print(nn.BCEWithLogitsLoss(reduction='sum')(p_bbox_xy[t_obj == 1.0], t_bbox_xy[t_obj == 1.0])/n_pos)
        # print("loss_xy(original): ", loss_xy)
        # print("loss_xy(new): ", loss_xy_new)
        # print( (t_obj == 1.0).shape)
        # #print(p_bbox_xy[t_obj == 1.0].shape)
        # print("loss_xy(after): ", loss_xy_func(p_bbox_xy[t_obj == 1.0], t_bbox_xy[t_obj == 1.0]).sum()/(len(p_bbox_xy[t_obj == 1.0])/2))
        # print("loss_xy(after): ", torch.mean(torch.sum(torch.sum(loss_xy_func(p_bbox_xy, t_bbox_xy), 2) * t_obj, 1)))

        #loss_wh = bbox_w * torch.mean(torch.sum(torch.sum(loss_wh_func(p_bbox_wh, t_bbox_wh), 2) * t_obj, 1))
        loss_wh = bbox_w * nn.MSELoss(reduction='sum')(p_bbox_wh[t_obj == 1.0], t_bbox_wh[t_obj == 1.0])/n_pos
        #print(nn.MSELoss(reduction='sum')(p_bbox_wh[t_obj == 1.0], t_bbox_wh[t_obj == 1.0])/n_pos)
        #print(torch.mean(torch.sum(torch.sum(loss_wh_func(p_bbox_wh, t_bbox_wh), 2) * t_obj, 1)))

        t_obj_pos = t_obj
        t_obj_neg = 1. - t_obj

        loss_obj_pos = obj_pos_w * nn.MSELoss(reduction='sum')(p_obj[t_obj == 1.0], t_obj[t_obj == 1.0]) / n_pos # torch.mean(torch.sum(loss_obj_func(p_obj, t_obj) * t_obj_pos, 1))
        loss_obj_neg = obj_neg_w * nn.MSELoss(reduction='sum')(p_obj[t_obj == 0.0], t_obj[t_obj == 0.0]) / n_pos

        # print("loss(org) ", torch.mean(torch.sum(loss_obj_func(p_obj, t_obj) * t_obj_neg, 1)))
        # print("loss(pos) ", nn.MSELoss(reduction='sum')(p_obj[t_obj == 0.0], t_obj[t_obj == 0.0]) / n_pos)
        # print("loss(neg) ", nn.MSELoss(reduction='sum')(p_obj[t_obj == 0.0], t_obj[t_obj == 0.0]) / n_neg)

        #loss_class = torch.mean(torch.sum(torch.sum(loss_class_func(p_class, t_class), 2) * t_obj, 1))

        loss_class = nn.BCEWithLogitsLoss(reduction='sum')(p_class[t_obj == 1.0], t_class[t_obj == 1.0])/n_pos
    ...
    #     t_obj = targets[..., 4]
    #
    #     responsible = (t_obj == 1.0)
    #     not_responsible = (t_obj == 0.0)
    #
    #     targets_responsible = targets[responsible]
    #     targets_not_responsible = targets[not_responsible]
    #
    #     preds_responsible = preds[responsible]
    #     preds_not_responsible = preds[not_responsible]
    #
    #     t_responsible_bbox_xy = targets_responsible[..., [0, 1]]
    #     t_responsible_bbox_wh = targets_responsible[..., [2, 3]]
    #     t_responsible_obj = targets_responsible[..., 4]
    #     t_not_responsible_obj = targets_not_responsible[..., 4]
    #     t_responsible_class = targets_responsible[..., 5:]
    #
    #     p_responsible_bbox_xy = preds_responsible[..., [0, 1]]
    #     p_responsible_bbox_wh = preds_responsible[..., [2, 3]]
    #     p_responsible_obj = torch.sigmoid(preds_responsible[..., 4])
    #     p_not_responsible_obj = torch.sigmoid(preds_not_responsible[..., 4])
    #     p_responsible_class = preds_responsible[..., 5:]
    #
    #     loss_xy_func = nn.BCEWithLogitsLoss(reduction='mean')
    #     loss_wh_func = nn.MSELoss(reduction='mean')
    #     loss_obj_func = nn.MSELoss(reduction='mean')
    #     loss_class_func = nn.BCEWithLogitsLoss(reduction='mean')
    #
    #     loss_xy = bbox_w * loss_xy_func(p_responsible_bbox_xy, t_responsible_bbox_xy)
    #     loss_wh = bbox_w * loss_wh_func(p_responsible_bbox_wh, t_responsible_bbox_wh)
    #
    #     # t_obj_pos = t_obj
    #     # t_obj_neg = 1. - t_obj
    #
    ...
    #     #
    #     # loss_obj = sigmoid_focal_loss_jit(
    #     #     p_obj,
    #     #     t_obj,
    ...
    #     # )
    #
    #     loss_obj_pos = obj_pos_w * loss_obj_func(p_responsible_obj, t_responsible_obj)
    #     loss_obj_neg = obj_neg_w * loss_obj_func(p_not_responsible_obj, t_not_responsible_obj)
    #
    #     loss_class = loss_class_func(p_responsible_class, t_responsible_class)
    #
    #     if self.num_classes <= 1:
    #         loss_class = torch.tensor(0.)
    #
