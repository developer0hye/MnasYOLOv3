...
def train(model, device):
    ...
    scheduler = tools_proposed.LRScheduler(optimizer=optimizer,
    ...
    for epoch in range(start_epoch, args.total_epoch):
        for images, bboxes_label_list in data_loader:
            ...
            # forward
            t0 = time.time()
            out, out_bbox = net(images)

            targets = tools_proposed.build_targets(model,
                                          bboxes_label_list,
            ...
            loss = model.yololoss(out, out_bbox, targets)
...
if __name__ == '__main__':
    ...
    from model_proposed import MnasYOLOv3
    yolo_net = MnasYOLOv3()
